document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('benevolatForm').addEventListener('submit', (event) => {
        ...
        // Envoyer le nom du bénévole et le nom du bénévolat au serveur pour enregistrement
        const nomBenevole = formData.get('nom');
        const nomBenevolat = formData.get('benevolat');
        fetch('http://localhost:5000/benevolat/submit', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ nom_benevole: nomBenevole, nom_benevolat: nomBenevolat }) // Envoyer les noms du bénévole et du bénévolat
        })
        .then(response => {
            if (!response.ok) {
